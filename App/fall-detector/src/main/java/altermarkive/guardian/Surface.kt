...
class Surface(context: Context?, attributes: AttributeSet?) : Zoom(context, attributes),
    ...
    private val cache = Cache(Tracker.BUFFER_COUNT, Tracker.N, 0, Double.NaN)
    ...
    override fun surfaceDraw(gfx: Canvas) {
        ...
        if (live.isChecked) {
            synchronized(Tracker.singleton.cache) {
                Tracker.singleton.cache.copyInto(cache)
            }
        }
        val position = cache.position
        chart(gfx, position)
    }
    ...
    private fun chart(gfx: Canvas, position: Int) {
        ...
        val height = height
        // Set dark background
        gfx.drawColor(context?.getColor(R.color.chartBackground) ?: -0x1000000)
    ...
    private fun grid(gfx: Canvas, chart: Chart, width: Int, height: Int) {
        val paint = paint(context?.getColor(R.color.chartGridLines) ?: -0x222223)
        // Vertical
        var i = 0
        while (i <= Tracker.N) {
            val x = x((Tracker.N - 1 - i).toDouble(), width.toDouble())
            gfx.drawLine(x.toFloat(), 0f, x.toFloat(), height.toFloat(), paint)
            i += 1000 / Tracker.INTERVAL_MS
        }
        // Horizontal
    ...
    private fun signals(
        ...
        for (signal in chart.signals) {
            ...
            val buffer = cache.buffers[signal.buffer]
            var lastX = Double.NaN
            var lastY = Double.NaN
            for (sample in 0 until Tracker.N) {
                val x = x(sample.toDouble(), width.toDouble())
                val value = buffer[(position + 1 + sample) % Tracker.N]
    ...
    companion object {
        val CHARTS = arrayOf(
            Chart(
                "Acc.", -2.0f, 2.0f, arrayOf(
                    Signal("X", -0x10000, Tracker.BUFFER_X),
                    Signal("Y", -0xff0100, Tracker.BUFFER_Y),
                    Signal("Z", -0xffff01, Tracker.BUFFER_Z)
                ), arrayOf(
            ...
            Chart(
                "Acc. X", -2.0f, 2.0f, arrayOf(
                    Signal("X", -0x10000, Tracker.BUFFER_X),
                    Signal("X LPF", -0xff0100, Tracker.BUFFER_X_LPF),
                    Signal("X HPF", -0xffff01, Tracker.BUFFER_X_HPF)
                ), arrayOf(
                    Threshold("+1G", -0x8100, +1.0),
                    Threshold("-1G", -0x8100, -1.0)
                )
            ...
            Chart(
                "Acc. Y", -2.0f, 2.0f, arrayOf(
                    Signal("Y", -0x10000, Tracker.BUFFER_Y),
                    Signal("Y LPF", -0xff0100, Tracker.BUFFER_Y_LPF),
                    Signal("Y HPF", -0xffff01, Tracker.BUFFER_Y_HPF)
                ), arrayOf(
                    Threshold("+1G", -0x8100, +1.0),
                    Threshold("-1G", -0x8100, -1.0)
                )
            ...
            Chart(
                "Acc. Z", -2.0f, 2.0f, arrayOf(
                    Signal("Z", -0x10000, Tracker.BUFFER_Z),
                    Signal("Z LPF", -0xff0100, Tracker.BUFFER_Z_LPF),
                    Signal("Z HPF", -0xffff01, Tracker.BUFFER_Z_HPF)
                ), arrayOf(
                    Threshold("+1G", -0x8100, +1.0),
                    Threshold("-1G", -0x8100, -1.0)
                )
            ...
            Chart(
                "Δ", -2.0f, 2.0f, arrayOf(
                    Signal("X D", -0x10000, Tracker.BUFFER_X_MAX_MIN),
                    Signal("Y D", -0xff0100, Tracker.BUFFER_Y_MAX_MIN),
                    Signal("Z D", -0xffff01, Tracker.BUFFER_Z_MAX_MIN)
                ), arrayOf(
                    Threshold("+1G", -0x8100, +1.0),
                    Threshold("-1G", -0x8100, -1.0)
                )
            ...
            Chart(
                "Thr.", -1.0f, 6.0f, arrayOf(
                    Signal("SV TOT", -0x10000, Tracker.BUFFER_SV_TOT),
                    Signal("SV D", -0xff0100, Tracker.BUFFER_SV_D),
                    Signal("SV MAX MIN", -0xffff01, Tracker.BUFFER_SV_MAX_MIN),
                    Signal("Z 2", -0x1000000, Tracker.BUFFER_Z_2)
                ), arrayOf(
                    Threshold("Fall: SV TOT", -0x10000, Tracker.FALLING_WAIST_SV_TOT),
                    Threshold("Impact: SV TOT, SV MAX MIN", -0x10000, Tracker.IMPACT_WAIST_SV_TOT),
                    Threshold("Impact: SV D", -0xff0100, Tracker.IMPACT_WAIST_SV_D),
                    Threshold(
                        "Impact: SV TOT, SV MAX MIN",
                        -0xffff01,
                        Tracker.IMPACT_WAIST_SV_MAX_MIN
                    ),
                    Threshold("Impact: Z 2", -0x1000000, Tracker.IMPACT_WAIST_Z_2)
                )
            ),
            Chart(
                "Evt.", -0.5f, 1.5f, arrayOf(
                    Signal("Falling", -0xff0100, Tracker.BUFFER_FALLING),
                    Signal("Impact", -0xffff01, Tracker.BUFFER_IMPACT),
                    Signal("Lying", -0x10000, Tracker.BUFFER_LYING)
                ), arrayOf()
            )
        )
        ...
        private fun x(x: Double, width: Double): Double {
            return x * (width - 1) / (Tracker.N - 1).toDouble()
        }
