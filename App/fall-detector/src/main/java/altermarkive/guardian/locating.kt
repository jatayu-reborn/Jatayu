...
//import android.provider.Controls.Secure
import androidx.core.app.ActivityCompat
import java.text.SimpleDateFormat
import java.util.Date
import java.util.Locale

class locating private constructor(private val context: Guardian) : LocationListener {
    ...
    override fun onLocationChanged(newLocation: Location) {
        ...
        synchronized(this) {
            ...
            if (deadline <= now && !replied) {
                val battery = Batterypercentage.level(context)
                val message: String
                if (accuracy(gps).isInfinite() && accuracy(network).isInfinite()) {
                    message = "Batterypercentage: ${battery}% Location: unknown"
                } else {
                    ...
                    message =
                        "Batterypercentage: $battery % Location ($time): $lat,$lon ~$accuracy m ^$altitude m $bearing deg $speed km/h http://maps.google.com/?q=${lat},${lon}"
                }
    ...
    companion object {
        ...
        internal var singleton: locating? = null

        fun initiate(guardian: Guardian) {
            if (null == singleton) {
                singleton = locating(guardian)
            }
        }
