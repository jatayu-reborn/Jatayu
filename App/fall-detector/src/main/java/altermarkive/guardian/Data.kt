...
class Data(private val guardian: Guardian) {
    ...
    private val queue = ConcurrentLinkedQueue<Bundle>()
    ...
    private fun sweep() {
        val unzipped: Array<String>? = DataBase.list(root.path, DataBase.FILTER_SQLITE)
        if (unzipped != null && unzipped.isNotEmpty()) {
            ...
            for (file in unzipped) {
                ...
                if (DataBase.zip(root.path, file)) {
                    DataBase.delete(root.path, file)
                    DataBase.delete(root.path, "${file}-journal")
                }
            }
        }
        val week = 7 * 24 * 60 * 60 * 1000
        val zipped: Array<String>? = DataBase.list(root.path, DataBase.FILTER_ZIP)
        if (zipped != null && zipped.isNotEmpty()) {
            Arrays.sort(zipped)
            for (file in zipped) {
                if (DataBase.age(root.path, file) + week < System.currentTimeMillis()) {
                    DataBase.delete(root.path, file)
                }
            }
        }
