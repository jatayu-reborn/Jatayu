...
object DataBase {
    ...
    internal fun zip(prefix: String, file: String): Boolean {
        ...
        val input: InputStream = try {
            ...
            Trace.d(TAG, "Failed to open '$file' for reading:\n$trace")
            return false
        }
        val output: OutputStream = try {
            ...
            val trace: String = android.util.Log.getStackTraceString(exception)
            Trace.d(TAG, "Failed to open '$file.zip' for writing:\n$trace")
            return false
        }
        ...
        try {
            ...
            val trace: String = android.util.Log.getStackTraceString(exception)
            Trace.e(TAG, "Failed to compress the file $file:\n${trace}")
            return false
        }
        var result = true
        try {
            ...
            val trace: String = android.util.Log.getStackTraceString(exception)
            Trace.e(TAG, "Failed to close the file $file:\n${trace}")
            result = false
        }
        try {
            output.close()
            ...
            val trace: String = android.util.Log.getStackTraceString(exception)
            Trace.e(TAG, "Failed to close the file $file.zip:\n${trace}")
            result = false
        }
        return result
    }
